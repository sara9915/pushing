...
{
    ros::init(argc, argv, "mpc_state_node");
    ros::NodeHandle nh;
    ros::Rate loop_rate(10);

    ros::ServiceServer service =
        nh.advertiseService("get_mpc_state", converter);
    ...
    std_msgs::Float64MultiArray mpc_state;
    mpc_state.data.resize(5);
    ...
    ros::Publisher mpc_state_pub = nh.advertise<std_msgs::Float64MultiArray>("mpc_state", 1);
    ...
    while (ros::ok() && activate_srv)
    {
        ...
        tf::StampedTransform transform_PB;
        ...
        try
        {
            ...
            listener.waitForTransform("/base_link", "/push_frame", tracker_pose.header.stamp, ros::Duration(0.5));
            listener.lookupTransform("/base_link", "/push_frame",
                                     tracker_pose.header.stamp, transform_PB);
        }
        ...
        else
        {
            ...
            T_PS0 =  transform.inverse() * transform_PB;
            ...
            mpc_state.data.at(0) = mpc_pose.x;
            mpc_state.data.at(1) = mpc_pose.y;
            mpc_state.data.at(2) = mpc_pose.theta;
            mpc_state.data.at(3) = T_PS0.getOrigin().getX();
            mpc_state.data.at(4) = T_PS0.getOrigin().getY();

            std::cout << mpc_state.data.at(4) << std::endl;
